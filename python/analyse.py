import numpy as np
from tqdm import tqdm
import math

def analyseResults(trace_array, textin_array, key):

  HW = [bin(n).count("1") for n in range(0, 256)]

  numtraces = np.shape(trace_array)[0] #total number of traces
  numpoint = np.shape(trace_array)[1] #samples per trace

  pt = textin_array
  

  bestguess, pge = singleThreaded(HW, numtraces, numpoint, pt, trace_array, key)
  print(bestguess)
  print(pge)

def intermediate(pt, keyguess):
    sbox = (
        )
        
    return sbox[pt ^ keyguess]

def singleThreaded( HW, numtraces, numpoint, pt, trace_array, key ):
  bestguess = [0] * 16
  cparefs = [0] * 16
  pge = [0]*16
  for bnum in tqdm(range(0,16), desc='Attacking subkeys', leave=False, position=2):
    cpaoutput = [0] * 256
    maxcpa = [0] * 256
    for kguess in range(0, 256):

      # Initialize arrays &amp; variables to zero
      sumnum = np.zeros(numpoint)
      sumden1 = np.zeros(numpoint)
      sumden2 = np.zeros(numpoint)

      hyp = np.zeros(numtraces)
      for tnum in range(0, numtraces):
        hyp[tnum] = HW[intermediate(pt[tnum][bnum], kguess)]

      # Mean of hypothesis
      meanh = np.mean(hyp, dtype=np.float64)

      # Mean of all points in trace
      meant = np.mean(trace_array, axis=0, dtype=np.float64)
      # For each trace, do the following
      for tnum in range(0, numtraces):
        hdiff = (hyp[tnum] - meanh)
        tdiff = trace_array[tnum] - meant
        sumnum = sumnum + (hdiff * tdiff)
        sumden1 = sumden1 + hdiff * hdiff
        sumden2 = sumden2 + tdiff * tdiff
          
      np.seterr(divide='ignore', invalid='ignore')
      cpaoutput[kguess] = sumnum / np.sqrt(sumden1 * sumden2)   
      maxcpa[kguess] = max(abs(cpaoutput[kguess]))

    bestguess[bnum] = np.argmax(maxcpa)
    cparefs[bnum] = np.argsort(maxcpa)[::-1]
    pge[bnum] = list(cparefs[bnum]).index(key[bnum])

  return bestguess, pge

class Analysis:
  class SubKey:
    def __init__(self, index, subkey):
      self.equA = np.array([np.zeros(5000)]*256)
      self.equB = np.array([np.zeros(5000)]*256)
      self.equC = np.array([np.zeros(5000)]*256)
      self.equD = np.array([np.zeros(5000)]*256)
      self.equE = np.array([np.zeros(5000)]*256)

      self.index = index
      self.subkey = subkey

      self.HW = [bin(n).count("1") for n in range(0, 256)]

    def intermediate(self, pt, keyguess):
      sbox = (
        )

      return sbox[pt ^ keyguess]

    def calc(self, trace, plaintext, traceNum):
      cpaOutput = [0] * 256
      cpaMax = [0] * 256
      for kGuess in range(0,256):
        self.equA[kGuess] += self.HW[self.intermediate(plaintext[self.index], kGuess)] * trace[:]
        self.equB[kGuess] += self.HW[self.intermediate(plaintext[self.index], kGuess)]
        self.equC[kGuess] += trace[:]
        self.equD[kGuess] += self.HW[self.intermediate(plaintext[self.index], kGuess)] ** 2
        self.equE[kGuess] += trace[:] ** 2
        numerator = ( ( traceNum * self.equA[kGuess] ) - ( self.equB[kGuess] * self.equC[kGuess] ) )
        denominator = np.sqrt( ( ( self.equB[kGuess] ** 2 ) - ( traceNum * self.equD[kGuess] ) ) * (( self.equC[kGuess] ** 2 ) - ( traceNum * self.equE[kGuess] ) ) )
        
        np.seterr(divide='ignore', invalid='ignore')
        cpaOutput[kGuess] = numerator / denominator
        cpaMax[kGuess] = max(abs(cpaOutput[kGuess]))
      bestGuess = np.argmax(cpaMax)
      cpaRef = np.argsort(cpaMax)[::-1]
      pge = list(cpaRef).index(self.subkey)
      return (bestGuess, pge)


  def __init__(self, key):
    self.tNum = 0
    self.subkeys = []
    for i in range(0, 16):
      self.subkeys.append(self.SubKey(i, key[i]))

  def calc(self):
    print()


  def addTrace(self, trace, plaintext):
    self.tNum += 1
    bestguess = []
    pge = []
    for subkey in self.subkeys:
      (bg, pg) = subkey.calc(trace, plaintext, self.tNum)
      bestguess.append(bg)
      pge.append(pg)
    print(bestguess)
    print(pge)
