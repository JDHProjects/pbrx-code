...
class Analysis:
  class SubKey:
    ...
    def intermediate(self, pt, kGuess):
      sbox = (
      ...
      return sbox[pt ^ kGuess]

    def calc(self, traceNum):
      cpaOutput = [0] * 256
      cpaMax = [0] * 256
      for kGuess in range(0,256):
        numerator = ( ( traceNum * self.equA[kGuess] ) - ( self.equB[kGuess] * self.equC[kGuess] ) )
        denominator = np.sqrt( ( ( self.equB[kGuess] ** 2 ) - ( traceNum * self.equD[kGuess] ) ) * (( self.equC[kGuess] ** 2 ) - ( traceNum * self.equE[kGuess] ) ) )
        
        np.seterr(divide='ignore', invalid='ignore')
        cpaOutput[kGuess] = numerator / denominator
        cpaMax[kGuess] = max(abs(cpaOutput[kGuess]))

      bestGuess = np.argmax(cpaMax)
      cpaRef = np.argsort(cpaMax)[::-1]
      pge = list(cpaRef).index(self.subkey)
      return (bestGuess, pge)

    def updateEquations(self, trace, plaintext):
      for kGuess in range(0,256):
        hwValue = self.HW[self.intermediate(plaintext[self.index], kGuess)]
        self.equA[kGuess] += hwValue * trace
        self.equB[kGuess] += hwValue
        self.equC[kGuess] += trace
